Rails.application.configure do
  ...
  config.x.isucon_admin_ssh_private_key_path = ENV.fetch('ISUXPORTAL_ISUCON_ADMIN_SSH_PRIVATE_KEY_PATH')
  ...
  config.x.rules_url = ENV.fetch('ISUXPORTAL_RULES_URL', 'https://isucon.net/archives/58657116.html')
  ...
  config.x.sentry.environment = ENV.fetch('MONITOR_ENV')

  config.x.region = ENV['AWS_REGION'] || 'ap-northeast-1'

  config.x.avatar_bucket_name = ENV.fetch('ISUXPORTAL_AVATAR_BUCKET')
